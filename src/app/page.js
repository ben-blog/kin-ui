                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {qType === 'yesno' && (
                      <>
                        <SmartBtn
                          label="예"
                          onClick={() => doAnswer(currentQ.id, '예')}
                          variant="primary"
                        />
                    ...
                    {qType === 'review' && (
                      <>
                        <SmartBtn
                          label="진행하자"
                          onClick={() => doAnswer(currentQ.id, '진행하자')}
                          variant="primary"
                        />
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {loading ? (
        ...
        <div style={{ flex: 1 }}>
          <WordCloud
            topics={topics}
            experiences={data?.recentExperiences || []}
            onTopicClick={(t) => setSelectedTopic(selectedTopic?.word === t.word ? null : t)}
          />
        </div>
      )}
      {selectedTopic && (
  ...
  const GrowthPanel = (
    <div>
      ...
      <div style={{ display: 'flex', gap: 28, marginBottom: 28, flexWrap: 'wrap' }}>
        ...
        <Num
          label="메모리 레이어"
          value={Object.values(memory).filter(Boolean).length}
          color="#555"
          size={isMobile ? 32 : 36}
        />
      </div>
      ...
      <div style={{ marginBottom: 20 }}>
        {Object.entries(expBySrc).map(([src, cnt]) => (
          <Bar
            ...
            value={cnt}
            max={Math.max(1, ...Object.values(expBySrc))}
            color={YELLOW}
          />
        ))}
      </div>
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            {loading
              ? '—'
              : `경험 ${data?.experienceCount || 0} · knowledge ${data?.knowledgeCount || 0}`}
          </div>
        </div>
